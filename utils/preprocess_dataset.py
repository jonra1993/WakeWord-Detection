...
class Dataset_Preprocessor:
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.train_meta = json.load(
            open(os.path.join(self.data_dir, "train.json"), "r")
        )
        ...
        for split, meta in splits:
            ...
            for i, file in enumerate(tqdm(meta, leave=True)):
                ...
                samples, _ = librosa.load(
                    os.path.join(self.data_dir, file["audio_file_path"]), sr=self.sr
                )
                ...
                if len(samples) > 0:
                    ...
                    frames = [
                        samples[index : index + vad_frame] for index in frame_timepoints
                    ]
                    # Pad last frame with zeros.
                    frames[-1] = np.pad(frames[-1], (0, vad_frame - len(frames[-1])))
                    frames_bytes = [
                        np.int16(frame * 32768).tobytes() for frame in frames
                    ]
                    frames_speech = [
                        self.vad.is_speech(frame_bytes, self.sr)
                ...
                try:
                    ...
                    speech_end = frame_timepoints[
                        -(list(reversed(new_frames_speech)).index(True) + 1)
                    ]
    ...
    def __isolate_speech_silero(self):
        ...
        print("hi5")
        new_meta = {}
        (get_speech_ts, save_audio, read_audio, *_) = self.utils
        ...
        for split, meta in splits:
            ...
            for file in tqdm(meta):
                check_index += 1
                #original_path = os.path.join(self.data_dir, file["audio_file_path"])
                original_path = file["audio_file_path"]
                new_path = os.path.join(self.out_dir, file["id"])
                samples = read_audio(original_path)

                if len(samples) > 0:
                    vad_segments = get_speech_ts(
                        samples,
                        self.vad,
                    ...
                    if len(vad_segments) > 0:
                        ...
                        new_file = {key: value for key, value in file.items()}                        
                        new_file["duration"] = speech_samples.shape[0] / self.sr
                        new_file["audio_file_path"] = new_path
                        new_meta.append(new_file)
    ...
    def examine_audio(
        self, path, samples, is_speech, frames, play_sound=True, plot=True
    ):
        if play_sound:
            import pydub
            import simpleaudio
            ...
            if is_speech is not None and plot:
                for index, label in enumerate(is_speech):
                    if label is True and index + 1 < len(frames):
                        plt.axvspan(frames[index], frames[index + 1], alpha=0.1)
    ...
    def enhance_train_set(self, original_path=None):
        ...
        negative_files = [
            file["audio_file_path"]
            for file in self.train_meta
            if not file["is_hotword"]
            and Path(os.path.join(original_path, file["audio_file_path"])).exists()
        ]
        for file in tqdm(self.train_meta, leave=True):
            if (
                ...
                pos_samples, _ = librosa.load(
                    os.path.join(original_path, file["audio_file_path"]), sr=self.sr
                )
                ...
                if speech:
                    ...
                    while neg_samples.size == 0:
                        replacement_index = np.random.randint(
                            len(negative_files)
                        )  # choose random negative sample
                        replacement_file = negative_files[replacement_index]
                        neg_samples, _ = librosa.load(
                            os.path.join(original_path, replacement_file), sr=self.sr
                        )

                    # Construct informative filename.
                    if offset:
                        save_path = (
                            "pos_" + file["id"] + "_neg_" + Path(replacement_file).stem
                        )
                    else:
                        save_path = (
                            "neg_" + Path(replacement_file).stem + "_pos_" + file["id"]
                        )
                else:
                ...
                if offset:
                    ...
                    new_neg_samples = np.append(
                        pos_samples_reduced, neg_samples[-num_samples_to_remove:]
                    )
                else:
                    pos_samples_reduced = pos_samples[num_samples_to_remove:]
                    new_neg_samples = np.append(
                        neg_samples[:num_samples_to_remove], pos_samples_reduced
                    )
                sf.write(
                    os.path.join(self.out_dir, "enhanced_train_negative", save_path),
                    new_neg_samples,
                    self.sr,
                )

                # Add metadata to be written out as json file.
                enhanced_meta.append(
                    {
                        "duration": len(new_neg_samples) / self.sr,
                        "worker_id": "n_a",
                        "audio_file_path": os.path.join(
                            "enhanced_train_negative", save_path
                        ),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Preprocesses audio data by isolating speech and "
        "enhancing train samples"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default="data/hey_snips_research_6k_en_train_eval_clean_ter",
        help="Directory with Hey Snips raw dataset",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="data_speech_isolated/",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Flag to examine problematic audio clips",
    )
    parser.add_argument(
        "--sample_rate", type=int, default=16000, help="Sample rate for audio (Hz)"
    )
    parser.add_argument(
        "--vad_type",
        type=str,
        default="silero",
        choices=["webrtc", "pyannotate", "silero"],
        help="Type of VAD used to extract speech",
    )
